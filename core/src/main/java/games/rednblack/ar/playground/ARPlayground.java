 ...
 */
public class ARPlayground extends GdxArApplicationListener implements GestureDetector.GestureListener {
    ...
    public void create() {
        ...
        modelAsset = new GLBLoader().load(Gdx.files.internal("boat.glb"));
    ...
    public boolean pan(float x, float y, float deltaX, float deltaY) {
        Gdx.app.log("Panning", x + " " + y + " " + deltaX + " " + deltaY + " ");
        if (modelScene != null) {
            rotateModel(deltaX * ROTATION_SPEED); // Adjust ROTATION_SPEED as needed
        }
        return true;
    }

    private float ROTATION_SPEED = 1.5f; // Adjust rotation speed here

    private void rotateModel(float degrees) {
        Quaternion rotation = new Quaternion().setFromAxis(Vector3.Y, degrees);
        targetDir.mul(rotation); // Update targetDir with the rotation
    }
